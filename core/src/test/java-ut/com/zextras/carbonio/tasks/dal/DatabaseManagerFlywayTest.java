...
class DatabaseManagerFlywayTest {
  ...
  void givenADatabaseNotInitializedGetDatabaseVersionShouldReturnZero() {
    // Given
    Mockito.when(flywayMock.info().current()).thenReturn(null);
  ...
  void
    ...
    // Given
    Mockito.when(flywayMock.info().current().getPhysicalLocation())
        .thenReturn("/db/migration/V1__FAKE-SCRIPT.sql");
